    ...
    <v-container>
        <v-layout justify-space-between align-center>
            ...
            <v-flex xs12 sm8 md2 text-xs-right>
                <v-btn icon class="ma-0" @click="setCardsMode('list')"><v-icon large :color="cardsType === 'list' ? 'red accent-1': ''">view_headline</v-icon></v-btn>
                <v-btn icon class="ma-0" @click="setCardsMode('tile')"><v-icon large :color="cardsType === 'tile' ? 'red accent-1': ''">apps</v-icon></v-btn>
            </v-flex>
        </v-layout>
